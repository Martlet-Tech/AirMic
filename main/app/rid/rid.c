...
// ── RID 广播任务 ─────────────────────────────────────────────
// 每 300ms 切换一条报文，三条报文一轮 900ms，满足 CAAC ≤1s 间隔
static void rid_task(void *arg)
{
    uint8_t basic_id[ODID_MSG_LEN];
    uint8_t location[ODID_MSG_LEN];
    uint8_t system_msg[ODID_MSG_LEN];

    build_basic_id(basic_id);
    build_location(location);
    build_system(system_msg);

    const uint8_t *msgs[3] = { basic_id, location, system_msg };
    const char    *names[3] = { "BASIC_ID", "LOCATION", "SYSTEM" };
    int idx = 0;

    // 先设好第一条报文数据，再启动广播
    set_adv_data_for_msg(msgs[0]);
    start_nonconn_adv();
    ESP_LOGI(TAG, "RID broadcast started");
    ...
    while (s_running) {
        ...
        // 停广播 → 换数据 → 重启广播
        ble_gap_adv_stop();
        set_adv_data_for_msg(msgs[idx]);
        start_nonconn_adv();

        ESP_LOGD(TAG, "RID tx: %s", names[idx]);
    }

    ble_gap_adv_stop();
    ESP_LOGI(TAG, "RID broadcast stopped");
    s_task = NULL;
    vTaskDelete(NULL);
}
...
{
    if (!s_enabled) {
        ESP_LOGI(TAG, "RID disabled, skip");
        return;
    }
    if (!s_uid_set) {
        ESP_LOGW(TAG, "UID not set, skip");
        return;
    }
    if (s_running || s_task != NULL) return;

    s_running = true;
    xTaskCreate(rid_task, "rid", 4096, NULL, 4, &s_task);
}
